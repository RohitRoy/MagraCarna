...
class Atom(object):
    ...
    def from_mmcif_row(cls, table, row_no):
        ...
        coos = np.array([table["_atom_site.Cartn_x"][row_no], 
                         table["_atom_site.Cartn_y"][row_no],
                         table["_atom_site.Cartn_z"][row_no]], 
                         dtype=np.float64)
        try:
    ...
    def to_pdb_record(self, atomnum, residue):
        atomnum, atomname = str(atomnum).rjust(5), self.name.ljust(3).rjust(4)
        resnum, resinsco = str(residue[1]).rjust(4), residue[2].rjust(1)
        resname, chainid = residue[0].ljust(3), residue[3].rjust(2)
        values = [atomnum, atomname, resname, chainid, resnum, resinsco]
        for value in self.coos:
            intval, decval = str(round(value, 3)).split(".")
            values.append("%s.%s" % (intval.rjust(4), decval.ljust(3, "0")))
        values.append(self.atype.rjust(24))
        #       6     11    10      3  24    26
        return "ATOM  %s %s %s%s%s%s   %s%s%s%s  " % tuple(values)
...
class Residue(object):
    ...
    def from_pdb_record(cls, line, iscor=False, asdict=False):
        ...
        if iscor:
            chain = None
            serno = props["serno"] = int(line[20:26])
            insco = None
        else:
...
class Structure(object):
    ...
    def __init__(self, structid, residues, grid, pairs=None, breaks=False):
        ...
        self.residues.sort(key=lambda res_: res_.entuple())
        self.chains = dict()
        self.chainend = 0
    ...
    def _chain_nucleotides(self):
        ...
        nuc_residues = list()
        for residue in self.residues:
            if residue.name in allnucs:
                residue.index = res_index = len(nuc_residues)
                nuc_residues.append(residue)
                ...
                residue.index = len(het_residues)
                het_residues.append(residue)
        self.chainend = chainend = len(nuc_residues)
        self.residues = nuc_residues + het_residues
        for residue in het_residues:
            residue.index += chainend
    ...
    def _find_segments(self, chain_residues):
        ...
        for residue in chain_residues[1:]:
            for segment in reversed(segments):
                if self.linked(residue, segments[-1][-1]):
                    segments[-1].append(residue)
                    break
                elif self.linked(segments[-1][0], residue):
                    segments[-1].insert(0, residue)
                    break
            else:
...
class HydrogenBondFinder(object):
    ...
    def __init__(self, ignore_H=True,
                 max_da_dist=Decimal('3.80'), min_da_dist=Decimal('2.10'),
                 max_ha_dist=Decimal('2.70'), min_ha_dist=Decimal('1.00'),
                 max_dha_ang=Decimal('180.'), min_dha_ang=Decimal('110.')):
        self.max_da, self.min_da = max_da_dist, min_da_dist
        self.max_ha, self.min_ha = max_ha_dist, min_ha_dist
        self.max_dha, self.min_dha = max_dha_ang, min_dha_ang
        self._set_verify_hbond(ignore_H)
        self._validate_parameters()

    def find_hbonds(self, atom, residue, structure):
        hbonds = list()
        is_donor = self._is_donor(residue, atom)
        is_accor = self._is_accor(residue, atom)
        if is_donor or is_accor:
            near_atom = structure.grid.get_nearby_atoms(self.max_da, atom)
            if is_donor:
                for acc_res, acc_atom, dist in near_atom:
                    if self._is_accor(acc_res, acc_atom)\
                      and self.verify_hbond(atom, residue, acc_atom, dist):
                        hbonds.append((acc_res, acc_atom))
            if is_accor:
                for don_res, don_atom, dist in near_atom:
                    if self._is_donor(don_res, don_atom)\
                      and self.verify_hbond(don_atom, don_res, atom, dist):
                        hbonds.append((don_res, don_atom))
        return hbonds
    ...
    def _validate_parameters(self):
        if not (self.MIN_DHA <= self.min_dha <= self.max_dha <= self.MAX_DHA
          and self.MIN_DA <= self.min_da <= self.max_da <= self.MAX_DA
          and self.MIN_HA <= self.min_ha <= self.max_ha <= self.MAX_HA):
            raise self.InvalidHBondParameters()

    def _verify_hbond_ignoring_H(self, donor, donor_residue, accor, da_dist):
        return self.min_da <= da_dist <= self.max_da
    ...
    def _verify_hbond_that_has_H(self, donor, donor_residue, accor, da_dist):
        ...
        for hatom_name in (hatom_names & set(donor_residue.atoms)):
            ...
            if self.min_dha <= dha_angle <= self.max_dha\
              and self.min_ha <= ha_distance <= self.max_ha\
              and self.min_da <= da_distance <= self.max_da:
                return True
        return False
...
class StructureFile(object):
    ...
    def extract_residues(self, limited_elements=None, limited_residues=None):
        """Converts file into list of Residue objects.

        Args:
            limited_elements: an optional set of elements.
                If not None, only those Atom objects are 
                read which have elements in this set.
            limited_residues: optional set of residue
                names. If not None, Atom objects from only
                these residue types are read.

        Returns:
            List of Residue objects read from the file.
        """
        residues = list()
        current_residue = Residue() # empty residue
        for atom, resprops in self._parser.extract_atoms():
            if (limited_elements and atom.atype not in limited_elements)\
              or (limited_residues and atom.resnm not in limited_residues):
                continue
            if not current_residue.matches(**resprops):
                current_residue = Residue(structid=self.structid, **resprops)
...
class SecondaryStructureManager(object):
    ...
    class BPFindParams(object):
        ...
        """
        def __init__(self, scratch=".", cleanup=True,
                     hbond_params=HydrogenBondFinder()):
            self.scratch = os.path.abspath(scratch)
            self.da_dist = hbond_params.max_da
    ...
    def __init__(self, structfilepath, run_bpfind=None, run_nuparm=None):
        if run_bpfind:
            ...
            self.clean_up = False
        self.validate_filename(structfilepath)
        with StructureFile(structfilepath, self._scratch) as structfile:
    ...
    def run_bpfind(self):
        ...
        with open(os.devnull, 'w') as NULL:
            exit_code = call(command, stdout=NULL, stderr=NULL)
    ...
    def extract_paired_structure(self, size=7.0, **limits):
        if None in [self.corfile, self.pairs]:
            self.read_bpfind_output_files()
        structresidues = self.structfile.extract_residues(**limits)
        ...
        return Structure(structid, structresidues, grid, self.pairs, True)
    ...
    def _grid_merged_residues(self, corresidues, structresidues, size):
        ...
        cor_resnames = {corres.name for corres in corresidues}
...
if __name__ == "__main__":
    grid_structures(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4])
